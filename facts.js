Module.register('facts', {
  ...
  // edit these
  defaults: {
    updateInterval: 60,		// how long a fact stays up in seconds
    ...
    facts: {
      ...
      tips: [
        ...
        "123456 is Planet Druidia's airlock password - it is also the most common computer password.",
  ...
  randomFact: function () {
    const facts = this.factArray()
    const index = this.randomIndex(facts)
    return facts[index]
  },

  // override getDom to display fact
  getDom: function () {
    const factText = this.randomFact()
